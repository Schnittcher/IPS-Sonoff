...
class IPS_SonoffSwitch4Ch extends IPSModule {
  ...
  public function Create() {
      ...
      for($n = 1;$n<5;$n++)
      {
        $this->RegisterVariableBoolean("SonoffStatus_Ch$n", "Status_Ch$n","~Switch");
        $this->EnableAction("SonoffStatus_Ch$n");
      }
  ...
  public function ReceiveData($JSONString) {
    ...
    IPS_LogMessage("SonoffSwitch",$data->Buffer);
	  ...
	  if (fnmatch("*POWER[1234]", $Buffer->TOPIC)) {
		  $this->SendDebug("Power", $Buffer->MSG,0);
      switch ($Buffer->MSG) {
        case $off:
          SetValue($this->GetIDForIdent("SonoffStatus_Ch" .substr(" $Buffer->TOPIC", -1)), 0);
          break;
        case $on:
          SetValue($this->GetIDForIdent("SonoffStatus_Ch" .substr(" $Buffer->TOPIC", -1)), 1);
          break;
      }
	  }
  ...
  public function setState($n /*channel*/, $Value) {
    SetValue($this->GetIDForIdent("SonoffStatus_Ch$n"), $Value);
  ...
  public function RequestAction($Ident, $Value) {
    switch ($Ident) {
      case fnmatch("SonoffStatus_Ch[1234]", $Ident):
        $result = $this->setState(substr($Ident, -1), $Value);
        break;
      default:
